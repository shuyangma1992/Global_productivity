...
hist(stock$median_T)
stock_T <- stock %>%
  mutate(median_T = case_when(median_T > 0.02 ~ 0.02,
                              median_T < -0.02 ~ -0.02,
                              TRUE ~ median_T))
...
T_map <- ggplot()+
  ...
  geom_point(data = stock_T, aes(x = centroid_lon, y = centroid_lat, color = median_T*100), size = 1)+
...
#combine
T_up <- ggarrange(T_map, labels = c("a"))
T_low <- ggarrange(T_x, T_y, nrow = 1, align = "hv",labels = c("b", "c"))
T_f <- ggarrange(T_up, T_low, heights = c(1.5,1), ncol = 1, align = "h")

# ggsave("Figures/GLMM results T map.PDF",device = cairo_pdf, width = 4.5,height = 5)
...
stock_CHL <- stock %>%
  mutate(median_CHL = case_when(median_CHL > 1 ~ 1,
                                median_CHL < -1 ~ -1,
                                TRUE ~ median_CHL))
CHL_map <- ggplot()+
  geom_map(map=world,data=world,aes(x=long,y=lat,map_id=region),fill="black")+
  # geom_spatvector(data = FAO_shp_major, fill="white",color="black")+
  # geom_sf(data = world,fill="black",color="black")+
  geom_point(data = stock_CHL, aes(x = centroid_lon, y = centroid_lat, color = median_CHL), size = 1)+
  scale_color_gradient2(low=brewer_pal(type="div",palette = "PuOr")(9)[9],high=brewer_pal(type="div",palette = "PuOr")(9)[1])+
  scale_x_continuous("Longitude", limits = c(-180,180), breaks = seq(-180,180,60), expand = c(0, 0))+
  scale_y_continuous("Latitude", limits = c(-85,85), breaks = seq(-90,90,30), expand = c(0, 0))+
  # geom_spatvector_text(data = FAO_shp_major, aes(label = F_CODE),
...
#combine
CHL_up <- ggarrange(CHL_map, labels = c("a"))
CHL_low <- ggarrange(CHL_x, CHL_y, nrow = 1, align = "hv", labels = c("b", "c"))
CHL_f <- ggarrange(CHL_up, CHL_low, heights = c(1.5,1), ncol = 1, align = "hv")

# ggsave("Figures/GLMM results CHL map.PDF",device = cairo_pdf, width = 4.5,height = 5)

f <- ggarrange(T_f, CHL_f, nrow = 2, align = "hv")

ggsave("Figures/GLMM results T CHL map.PDF",device = cairo_pdf, width = 5,height = 8)
