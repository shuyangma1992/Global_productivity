...
# Figure about future winner and loser, use IPSL, 2090s - 2020s results
...
unique(productivity_change_90s_20s$scientificname) # 265 stocks
...
f_90s_IPSL <- ggplot(productivity_change_90s_20s) +
...
ggsave("Figures/winner_loser_2090s_IPSL.pdf", device = cairo_pdf, width = 6, height = 3)



# XXX 2050s - 2020s IPSL ---------------------------------------------------------
# 2050s - 2020s
...
# combine data
productivity_change_50s_20s <- left_join(productivity_change_50s_20s, stock_success)

# number of stocks
numbers <- productivity_change_50s_20s %>%
  mutate(significance = ifelse(p_change_mean > 0 & p_change_2.5 > 0,
    1,
    ifelse(p_change_mean < 0 & p_change_97.5 < 0,
      -1,
...
# 2050s - 2020s
f_50s <- ggplot(productivity_change_50s_20s) +
  geom_hline(yintercept = 0) +
  geom_point(aes(x = stockid, y = p_change_mean, color = ssp), show.legend = F, size = 0.25) +
  scale_color_manual(values = c(
    "ssp126" = mypal[3],
  ...
  annotate("text", x = 450, y = 0.10, label = paste(
    "Top 5 winners",
    winner_5$stocklong[5],
    winner_5$stocklong[4],
    winner_5$stocklong[3],
  ...
  annotate("text", x = 100, y = -0.10, label = paste(
    "Top 5 losers",
    loser_5$stocklong[1],
    loser_5$stocklong[2],
    loser_5$stocklong[3],
  ...
  scale_x_discrete("Stock (arranged by productivity change under SSP 5-8.5)", expand = c(0, 0)) +
  scale_y_continuous("Productivity change", limits = c(-0.15, 0.15), expand = c(0, 0)) +
  mytheme

ggsave("Figures/winner_loser_2050s_IPSL.pdf", device = cairo_pdf, width = 6, height = 3)
...
productivity_change_50s_20s <- productivity_change_50s_20s %>%
  ...
  drop_na()
unique(productivity_change_50s_20s$stockid) # 608 stocks
...
ggsave("Figures/winner_loser_2050s_MPI.pdf", device = cairo_pdf, width = 6, height = 3)
...
productivity_change_50s_20s <- productivity_change_50s_20s %>%
  ...
  drop_na()
unique(productivity_change_50s_20s$stockid) # 614 stocks
...
ggsave("Figures/winner_loser_2050s_Nor.pdf", device = cairo_pdf, width = 6, height = 3)
