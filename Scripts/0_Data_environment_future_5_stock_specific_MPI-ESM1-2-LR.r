...
#data files
var_files <- list.files("D:/CMIP6/Compiled/MPI-ESM1-2-LR compiled/")
...
env_data_run_parallel <- foreach(i=1:nrow(stock), .combine = "rbind",.packages = c("tidyverse","geojsonR","terra","tidyterra")) %do% {
  ...
  env_dat_stock <- foreach(j=var_files, .combine = "rbind", .packages = c("tidyverse","geojsonR","terra","tidyterra")) %dopar% {
    ...
    var_loop <- rast(readRDS(paste0("D:/CMIP6/Compiled/MPI-ESM1-2-LR compiled/",j)))
    # plot(var_loop,1)
  ...
  write_rds(env_dat_stock,file=paste0("Data/CMIP6 stocks/MPI-ESM1-2-LR/", stock_GRSF_uuid_loop,"_chl.rds"))
...
env_data_run_parallel <- foreach(i=1:nrow(stock), .combine = "rbind",.packages = c("tidyverse","geojsonR","terra","tidyterra")) %do% {
  ...
  env_dat_stock <- foreach(j=var_files, .combine = "rbind", .packages = c("tidyverse","geojsonR","terra","tidyterra")) %dopar% {
    ...
    env_dat_loop <- var_mean_loop %>% 
      ...
      rename("thetao"=mean) %>% 
      mutate(uuid=stock_GRSF_uuid_loop) %>% 
      mutate(ssp=str_sub(j,27,32))
    
  }
  
  write_rds(env_dat_stock,file=paste0("Data/CMIP6 stocks/MPI-ESM1-2-LR/", stock_GRSF_uuid_loop,"_thetao.rds"))
...
env_data_run_parallel <- foreach(i=1:nrow(stock), .combine = "rbind",.packages = c("tidyverse","geojsonR","terra","tidyterra")) %do% {
  ...
  write_rds(env_dat_stock,file=paste0("Data/CMIP6 stocks/MPI-ESM1-2-LR/", stock_GRSF_uuid_loop,"_mlotst.rds"))
...
# test --------------------------------------------------------------------
a <- read_rds("Data/CMIP6 stocks/MPI-ESM1-2-LR/fe0c21c8-8872-36bc-8063-d3fc3fbe9dff_chl.rds")
