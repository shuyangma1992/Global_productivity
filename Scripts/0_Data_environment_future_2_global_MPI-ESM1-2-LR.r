...
# CMIP 6 data compile MPI-ESM1-2-LR ---------------------------------------
#basic information
nc <- nc_open("thetao_Omon_MPI-ESM1-2-LR_ssp126_r1i1p1f1_gn_201501-203412.nc")



data <- tidync("thetao_Omon_MPI-ESM1-2-LR_ssp126_r1i1p1f1_gn_201501-203412.nc") 
...
time <- data %>% 
  activate("D0") %>% 
  hyper_tibble() %>% 
  mutate(time = as_date(time, origin = "1850-1-1 00:00:00")) #make a little change
...
#0-50m temperature average
temp <- temp %>% 
  group_by(time, i, j) %>% 
  summarise(thetao = mean(thetao, na.rm = T)) %>% 
  ungroup()

#combine data
temp <- left_join(temp, lon_lat) %>% 
  mutate(time = as_date(time, origin = "1850-1-1 00:00:00"))

temp <- temp %>% 
  mutate(longitude = longitude-180) %>% #change longitude
  select(longitude, latitude, time, thetao)
...
#loop
temp_even_grid <- rast()
for (i in 1:240) { #120 months
  
  # i=2
  time_loop <- time[i,1]
  temp_loop <- temp %>% 
    filter(time %in% time_loop)
  
  #original coordinates
  uneven_grid <- as.matrix(temp_loop[,1:2]) 
  
  temp_even_grid_loop <- rasterize(uneven_grid,even_grid,value=temp_loop$thetao,fun=mean) #get even grid
  # plot(temp_even_grid_loop)
  names(temp_even_grid_loop) <-time_loop$time #change the name of layer as time
  temp_even_grid <- c(temp_even_grid,temp_even_grid_loop) #combine the grids
  print(i)
}

#now we get a raster file with even grid, it has 1020 layers, standing for 1020 months.
plot(temp_even_grid,7) 
names(temp_even_grid)
saveRDS(temp_even_grid,"ssp126_MPI.rds")
